...
function onFormSearch(e) {
    ...
    if (!query) return (0, _notiflixNotifyAio.Notify).failure("Please specify your search query.");
    (0, _fetchImagesDefault.default)(query, currentPage, perPage).then(({ data  })=>{
        if (data.totalHits === 0) {
        ...
        (0, _markupDefault.default)(data.hits);
        if (data.totalHits < perPage && data.totalHits === "") return loadBtnEl.classList.add("is-hidden");
...
function onLoadMore() {
    ...
    (0, _fetchImagesDefault.default)(query, currentPage, perPage).then(({ data  })=>{
        ...
        (0, _markupDefault.default)(data.hits);
        galleryLightbox.refresh();
        if (currentPage < totalPages) return loadBtnEl.classList.remove("is-hidden");
        else {
            loadBtnEl.classList.add("is-hidden");
...
},{"./css/styles.css":"1CY4s","notiflix/build/notiflix-notify-aio":"eXQLZ","./fetchImages":"8Bjl1","./markup":"avFhm","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc"}],"1CY4s":[function() {},{}],"eXQLZ":[function(require,module,exports) {
var global = arguments[3];
/*
...
async function fetchImg(query, currentPage, perPage) {
    const response = await (0, _axiosDefault.default).get(`?key=${KEY}&q=${query}&image_type=photo&orientation=horizontal&safesearch=true&page=${currentPage}&per_page=${perPage}`);
    return response;
}
...
function galleryRender(images) {
    const imgCardRender = images.map((img)=>{
    ...
    galleryEl.insertAdjacentHTML("beforeend", imgCardRender);
}
exports.default = galleryRender;
...
},{}],"kaxSc":[function() {},{}]},["ShInH","8lqZg"], "8lqZg", "parcelRequired7c6")
